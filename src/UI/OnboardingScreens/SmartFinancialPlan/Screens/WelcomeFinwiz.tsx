...
const WelcomeFinwiz = ({navigation}) => {
  ...
  const [steps, setsteps] = useState([
    {
      ...
      description:
        'To securely link your bank account, click the button below to connect via Plaid. This will enable seamless and secure syncing of your financial data.',
      selected: false,
      color: '#9747FF',
    ...
    {
      step: 'Step 2',
      title: 'Set Your Budget Plan',
      description:
        'Take control of your finances! Click below to set up your personalized budget plan and start tracking your expenses with ease.',
      selected: false,
      color: '#21014E',
  ...
  const createLinkToken = React.useCallback(async () => {
    await fetch(
      `https://api-finwiz.softsquare.io/api/user/plaid-generate-link-token`,
      {
        method: 'POST',
        headers: {
  ...
  const checkUserQuestionAnswers = async () => {
    ...
    fetch('https://api-finwiz.softsquare.io/api/user/get-user-question', {
      ...
      .then(response => response.json())
      .then(data => {
        console.log('ALL ANSWERS OF QUESTIONS`1', data.data);
      ...
      <SafeAreaView style={{flex: 1}}>
        ...
        <View
          ...
          <View
            ...
            />
            <View>
              <Text style={{fontSize: 24, fontWeight: '600', color: '#9747FF'}}>
                Welcome to Finwiz
              </Text>
              <Text
                style={{
                ...
                Lets get you set up, complete your profile and get started.
              </Text>
            </View>
          </View>
          <View style={{position: 'absolute', right: -40, top: 32}}>
            <Image
              style={{width: 130, height: 130, opacity: 0.2}}
              source={require('../../../../assets/Images/logo.png')}
            />
          </View>
        </View>
        <View style={{flex: 0.7, paddingHorizontal: 16}}>
          {steps.map((item, index) => (
            <StepsComp
              errorShow3={errorShow3}
              publicTokenApiCall={publicTokenApiCall}
              errorShow2={errorShow2}
              errorShow={errorShow}
              index={index}
              error={errorText}
              comp={comp}

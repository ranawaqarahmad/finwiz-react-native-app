...
const RetirementScreenB = () => {
  const [selectedOption, setSelectedOption] = useState('');
  const navigation = useNavigation();

  const handleClickArrow = () => {
    navigation.navigate('FinancialScreenA');
  };
  ...
  return (
    <View style={styles.mainview}>
      <Progress.Bar progress={0.72727272} width={null} color="#9747FF" />
      {/* cross image design */}
      <View style={styles.submain}>
        ...
        <View style={{ paddingBottom: 30 }}>
          ...
          What kind of retirement lifestyle are you aiming for?
          </Text>
        ...
        <View>
          ...
          {['Frugal', 'Moderate', 'Comfortable'].map((option, index) => (
            <TouchableOpacity
              key={index}
              style={{
                padding: 10,
...
export default RetirementScreenB

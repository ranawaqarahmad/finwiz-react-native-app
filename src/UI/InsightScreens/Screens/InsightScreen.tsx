...
const InsightScreen = () => {
  ...
  const normalizeAndSetState = (data) => {
    ...
    const normalizedData = data.map((item, index) => {
      ...
      return { ...item, adjustedValue: normalizedTotal, enabled: index == data.length - 1 ? true : false };
    });
    ...
    <View style={{ backgroundColor: '#FFFFFF', flex: 1 }}>
      <View style={{ flexDirection: 'row', marginHorizontal: 16 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
          <Text style={{ color: 'black', fontSize: 18, fontWeight: '600' }}>Insights</Text>
        </View>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', columnGap: 8, }}>
          ...
          <TouchableOpacity onPress={() => console.log(categories)
            ...
            <Image source={require('../../../assets/Images/calendarblack.png')}
...

      {
        loader ?
          ...
          <ScrollView>
            <Text style={{ color: 'black', fontSize: 16, fontWeight: 'bold', marginTop: 16, marginHorizontal: 16 }}>{getMonthName(monthDetails.month)}</Text>
            <Text style={{ color: 'black', fontSize: 24, fontWeight: '600', marginBottom: 0, margin: 16 }}>{'USD $'}{monthDetails.total}</Text>
            <Text style={{ color: '#4B5563', fontSize: 16, fontWeight: '600', marginHorizontal: 14 }}>{'Total Spending'}</Text>


            <View style={{ flexDirection: 'row', height: 250, marginHorizontal: 16, columnGap: 16, marginTop: 24, alignItems: 'flex-end' }}>
              {allMonths.map((item, index) =>
                <View key={index} style={{ alignItems: 'center' }}>



                  <TouchableOpacity onPress={() => updateEnabledAtIndex(index)} style={{ height: item.adjustedValue, backgroundColor: item.enabled ? '#7E3AF2' : '#CABFFD', width: 50, borderRadius: 16 }}></TouchableOpacity>

                  <Text style={{ fontSize: 12, fontWeight: item.enabled ? '700' : 'normal', marginTop: 8, marginHorizontal: 16, color: item.enabled ? 'black' : '#4B5563' }}>{getMonthName(item.month)}</Text>
                  <View style={{ width: 10, height: 10, borderRadius: 100, backgroundColor: item.enabled ? '#7E3AF2' : 'transparent', marginTop: 2 }}></View>
                </View>)}
              ...
              {nextMonths.length < 5 && (
                ...
                nextMonths.map((item, index) =>
                  <View key={index + 100} style={{ alignItems: 'center' }}>
                    ...
                    <Text style={{ fontSize: 12, fontWeight: item.enabled ? '700' : 'normal', marginTop: 8, marginHorizontal: 16, color: item.enabled ? 'black' : '#4B5563' }}>{getMonthName(item)}</Text>
                    <View style={{ width: 10, height: 10, borderRadius: 100, backgroundColor: item.enabled ? '#7E3AF2' : 'transparent', marginTop: 2 }}></View>
                  </View>)
            ...
            <View style={{
              ...
              <View style={{ justifyContent: 'center', flex: 1, alignItems: 'center' }}>
                <Text style={{ color: 'black', fontSize: 16, fontWeight: 'bold' }}>$4500</Text>
                <Text style={{ color: '#6B7280', fontSize: 14, fontWeight: '600' }}>Savings</Text>
              </View>
              <View style={{ justifyContent: 'center', flex: 1, alignItems: 'center' }}>
                <Text style={{ color: 'black', fontSize: 16, fontWeight: 'bold' }}>$4500</Text>
                <Text style={{ color: '#6B7280', fontSize: 14, fontWeight: '600' }}>Investments</Text>
              </View>
            </View>

            <Text style={{ color: 'black', fontSize: 16, fontWeight: 'bold', margin: 16 }}>Categories</Text>

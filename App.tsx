      ...
      .then(data => {
        ...
        if (data.status == 'true') {
          ...
          console.log(
            ...
            data.data.user_question_answer_count,
          );
          console.log('ALL ANSWERS OF QUESTIONS', data.data.total_questions);

          if (
            data.data.user_question_answer_count == data.data.total_questions
          ) {
  ...
  return (
    <NavigationContainer>
      {loader ? (
        ...
        selector.basicInfoCompleted &&
        selector.phoneVerified &&
        selector.syncAccountDone &&
        selector.accountId &&
        selector.setupBudgetPlanDone ? (
        <BtmNav />
